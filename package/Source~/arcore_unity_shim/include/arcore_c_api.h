...
/// A detected planar image (@ref ownership "reference type, long-lived").
...
///     the result.
void ArCoreApk_checkAvailability(void* env, void* application_context,
...
///     installation.
ArStatus ArCoreApk_requestInstall(void* env, void* application_activity,
...
/// - #AR_UNAVAILABLE_USER_DECLINED_INSTALLATION if the user previously declined
///     installation.
ArStatus ArCoreApk_requestInstallCustom(void* env, void* application_activity,
...
/*DYNAMITE_FUNCTION_180405000*/
void ArConfig_setCloudAnchorMode(const ArSession* session, ArConfig* config,
                                 ArCloudAnchorMode cloud_anchor_mode);

/// Sets the augmented image database in the session configuration.
/*DYNAMITE_FUNCTION_180405000*/
void ArConfig_setAugmentedImageDatabase(
    const ArSession* session, ArConfig* config,
    const ArAugmentedImageDatabase* augmented_image_database);

/// Returns the augmented image database from the session configuration.
/*DYNAMITE_FUNCTION_180405000*/
...
///   produce the image
/*DYNAMITE_FUNCTION_180226000*/
ArStatus ArFrame_acquireCameraImage(ArSession* session, ArFrame* frame,
                                    ArImage** out_image);
...
///     the point.
/*DYNAMITE_FUNCTION_180129000*/
void ArPoint_getOrientationMode(const ArSession* session, const ArPoint* point,
...
/// Returns the pose of the center of the detected augmented image. The pose's
/// transformed +Y axis will be point normal out of the augmented image.
/*DYNAMITE_FUNCTION_180405000*/
void ArAugmentedImage_getCenterPose(const ArSession* session,
                                    const ArAugmentedImage* augmented_image,
                                    ArPose* out_pose);

/// Retrieves the length of this augmented image's boundary rectangle measured
...
/// Returns the (zero-based) index of this augmented image from its originating
/// augmented image database.
/*DYNAMITE_FUNCTION_180405000*/
...
/// *out_augmented_image_name to point to that string. The caller is expected to
/// release the string using ArString_release when the string is no longer
/// needed.
/*DYNAMITE_FUNCTION_180405000*/
...
/// Creates a new empty augmented image database.
/*DYNAMITE_FUNCTION_180405000*/
void ArAugmentedImageDatabase_create(
    const ArSession* session,
    ArAugmentedImageDatabase** out_augmented_image_database);

/// Creates a new augmented image database from a byte array. The contents of
/// the byte array must have been generated by the database builder developer
/// tool, or at runtime from ArAugmentedImageDatabase_serialize.
///
/// @return #AR_SUCCESS or any of:
...
/// Serializes an augmented image database to a byte array.
///
...
/// from an array of grayscale pixel values. Returns the positional index of the
/// image within the image database.
///
/// If the physical size of the image is known, use
/// ArAugmentedImageDatabase_addImageWithPhysicalSize instead, to improve
/// average image detection time.
///
/// For images added via ArAugmentedImageDatabase_addImage, ARCore estimates the
/// image's physical size and pose at runtime when the image is visible and is
/// being tracked. This extra estimation step will require the user to move
/// their device to view the image from different viewpoints before the image
...
ArStatus ArAugmentedImageDatabase_addImage(
    const ArSession* session,
    ArAugmentedImageDatabase* augmented_image_database, const char* image_name,
    const uint8_t* image_grayscale_pixels, int32_t image_width_in_pixels,
    int32_t image_height_in_pixels, int32_t image_stride_in_pixels,
    int32_t* out_index);
...
/// Returns the positional index of the image within the image database.
///
/// If the physical size of the image is not known, use
/// ArAugmentedImageDatabase_addImage instead, at the expense of poorer average
/// image detection time.
///
/// For images added via ArAugmentedImageDatabase_addImageWithPhysicalSize,
/// ARCore can estimate the pose of the image at runtime as soon as ARCore
/// detects the image, without requiring the user to move the device to view the
/// image from different viewpoints. Note that ARCore will refine the estimated
/// physical size and pose of the image as it is viewed from different
/// viewpoints.
///
/// The image name is expected to be a null-terminated string in UTF-8 format.
///
...
/// Returns the number of images in the augmented image database.
/*DYNAMITE_FUNCTION_180405000*/
...
/// Releases memory used by a augmented image database.
/*DYNAMITE_FUNCTION_180405000*/
