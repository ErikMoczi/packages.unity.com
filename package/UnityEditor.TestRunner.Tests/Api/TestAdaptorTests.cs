...
{
    [Test]
    public void TestAdaptor_TestWithChildIndex_AddsSuffixToTestName()
    {
        var test = new TestMock { FullName = "Test"};
        test.Properties.Set("childIndex", 1);

        var testAdaptor = new TestAdaptor(test);

        Assert.That(testAdaptor.FullName, Is.EqualTo("Test GeneratedTestCase1"));
    }

    [Test]
    public void TestAdaptor_TestFromTestCaseSourceWithChildIndex_AddsSuffixToTestName()
    {
        var test = new TestMock { FullName = "Test(Type)"};
        test.Properties.Set("childIndex", 1);

        var testAdaptor = new TestAdaptor(test);

        Assert.That(testAdaptor.FullName, Is.EqualTo("Test(Type GeneratedTestCase1)"));
    }

    [Test]
    public void TestAdaptor_TestWithChildIndexInRemoteTestData_AddsSuffixToTestName()
    {
        var test = new TestMock { FullName = "Test"};
        test.Properties.Set("childIndex", 1);
        var remoteTestData = new RemoteTestData(test);

        var testAdaptor = new TestAdaptor(remoteTestData);

        Assert.That(testAdaptor.FullName, Is.EqualTo("Test GeneratedTestCase1"));
    }

    [Test]
    public void TestAdaptor_TestFromTestCaseSourceWithChildIndexInRemoteTestData_AddsSuffixToTestName()
    {
        var test = new TestMock { FullName = "Test(Type)"};
        test.Properties.Set("childIndex", 1);
        var remoteTestData = new RemoteTestData(test);
