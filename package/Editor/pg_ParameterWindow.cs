...
{
	class pg_ParameterWindow : EditorWindow
	{
		internal pg_Editor editor;
		...
		void OnGUI()
		{
			...
			EditorGUI.BeginChangeCheck();
			int majorLineIncrement = EditorPrefs.GetInt(pg_PreferenceKeys.MajorLineIncrement, 10);
			...
			if(EditorGUI.EndChangeCheck())
			{
				EditorPrefs.SetInt(pg_PreferenceKeys.MajorLineIncrement, majorLineIncrement);
				pg_GridRenderer.majorLineIncrement = majorLineIncrement;
				pg_Editor.DoGridRepaint();
			}

			editor.ScaleSnapEnabled = EditorGUILayout.Toggle("Snap On Scale", editor.ScaleSnapEnabled);

			SnapUnit _gridUnits = (SnapUnit)(EditorPrefs.HasKey(pg_PreferenceKeys.GridUnit) ? EditorPrefs.GetInt(pg_PreferenceKeys.GridUnit) : 0);
			...
			if( EditorGUI.EndChangeCheck() )
			{
				EditorPrefs.SetInt(pg_PreferenceKeys.GridUnit, (int) _gridUnits);
			...
			if( tmp != editor.predictiveGrid )
			{
				...
				EditorPrefs.SetBool(pg_PreferenceKeys.PredictiveGrid, tmp);
			}
