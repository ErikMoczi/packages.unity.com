...
namespace ProGrids.Editor
{
	...
	/// </summary>
	[Serializable]
	class pg_VersionInfo : IEquatable<pg_VersionInfo>, IComparable<pg_VersionInfo>
	{
		[SerializeField]
		int m_Major = -1;
		...
		public pg_VersionInfo()
		{
		}

		public pg_VersionInfo(string formatted, string date = null)
		{
			pg_VersionInfo parsed;
		...
		public pg_VersionInfo(int major, int minor, int patch, int build = 0, VersionType type = VersionType.Development, string date = "", string metadata = "")
		{
			m_Major = major;
			m_Minor = minor;
			m_Patch = patch;
		...
		{
			return o is pg_VersionInfo && this.Equals((pg_VersionInfo) o);
		}
		...
		public bool Equals(pg_VersionInfo version)
		{
		...
		public int CompareTo(pg_VersionInfo version)
		{
			const int GREATER = 1;
			const int LESS = -1;
		...
		/// <summary>
		/// Create a pg_VersionInfo type from a string formatted in valid semantic versioning format.
		/// https://semver.org/
		...
		/// <returns></returns>
		public static bool TryGetVersionInfo(string input, out pg_VersionInfo version)
		{
			version = new pg_VersionInfo();
			bool ret = false;
