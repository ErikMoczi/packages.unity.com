        ...
        internal override void ProcessGroup(AddressableAssetSettings settings, AddressableAssetSettings.AssetGroup assetGroup, List<AssetBundleBuild> bundleInputDefs, List<ResourceLocationData> locationData)
        {
            foreach (var e in assetGroup.entries)
            {
                ...
                foreach (var s in assets)
                {
                    ...
                    if (s.isScene)
                    {
                        locationData.Add(new ResourceLocationData(s.address, s.guid, assetPath, typeof(SceneProvider).FullName, ResourceLocationData.LocationType.String, 0, typeof(UnityEngine.SceneManagement.Scene).FullName, null));
                        var indexInSceneList = IndexOfSceneInEditorBuildSettings(new GUID(s.guid));
                        if (indexInSceneList >= 0)
                            locationData.Add(new ResourceLocationData(indexInSceneList.ToString(), s.guid, assetPath, typeof(SceneProvider).FullName, ResourceLocationData.LocationType.Int, 0, typeof(UnityEngine.SceneManagement.Scene).FullName, null));
                    }
                    else
                    {
                        locationData.Add(new ResourceLocationData(s.address, s.guid, assetPath, typeof(SceneProvider).FullName, ResourceLocationData.LocationType.String, 0, typeof(UnityEngine.SceneManagement.Scene).FullName, null));
