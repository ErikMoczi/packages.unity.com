...
namespace ProGrids.Editor
{
	...
	static class pg_VersionUtil
	{
		...
		{
			pg_VersionInfo version = pg_Version.Current;
		...
		public static pg_VersionInfo GetVersionFromChangelog()
		{
			var changelogAsset = pg_EditorUtility.LoadInternalAsset<TextAsset>("About/changelog.txt");

			if (changelogAsset == null)
				return new pg_VersionInfo("changelog not found", "");

			Match m = Regex.Match(changelogAsset.text, "(?<=# ProBuilder )[0-9\\-a-zA-Z.]*", RegexOptions.Multiline);

			pg_VersionInfo version;
			pg_VersionInfo.TryGetVersionInfo(m.Value, out version);
			return version;
		}
		...
		/// <returns></returns>
		public static bool FormatChangelog(string raw, out pg_VersionInfo version, out string formattedChangelog)
		{
			bool success = true;
			...
			try
			{
				...
				success = pg_VersionInfo.TryGetVersionInfo(versionMatch.Success ? versionMatch.Value : split[1].Split('\n')[0], out version);
			}
			catch
			{
