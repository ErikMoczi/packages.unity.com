...
namespace UnityEditor.ResourceManagement.Diagnostics
{
    internal class GraphLayerLabel : GraphLayerBase
    {
        ...
        internal GraphLayerLabel(int stream, string name, string desc, Color color, System.Func<int, string> func) : base(stream, name, desc, color) { m_labelFunc = func; }
            ...
            if (stream != null)
            {
                ...
                if (inspectFrame != endTime)
                {
                    ...
                    if (val > 0)
                    {
                        ...
                        GUI.DrawTexture(labelRect, EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill, true, 0, new Color(0, 0, 0, .75f), 50, 5);
