...
namespace Unity.VectorGraphics
{
    public static partial class VectorUtils
    {
        ...
        /// <param name="root">The root node of the hierarchy to tessellate</param>
        ...
        public static List<Geometry> TessellateNodeHierarchy(SceneNode root, TessellationOptions tessellationOptions, Dictionary<SceneNode, float> nodeOpacities = null)
        {
            var nodeGeoms = new Dictionary<SceneNode, List<Geometry>>();

            foreach (var nodeWorldTransformed in WorldTransformedSceneNodes(root, nodeOpacities))
            {
                var node = nodeWorldTransformed.node;
                ...
                {
                    g.color.a *= nodeWorldTransformed.worldOpacity;
                    g.worldTransform = nodeWorldTransformed.worldTransform;
                    g.unclippedBounds = Bounds(g.vertices);
                }

                List<Geometry> prevGeoms;
                if (nodeGeoms.TryGetValue(node, out prevGeoms))
                {
                    // This node may already be processed when instantiating symbols,
                    // so we just append the new geoms in this case.
                    prevGeoms.AddRange(geoms);
                }
                else
                {
                    nodeGeoms[node] = geoms;
                }
            }

            ClipNodeHierarchy(root, nodeGeoms, tessellationOptions);

            var result = new List<Geometry>();
            foreach (var nodeWorldTransformed in WorldTransformedSceneNodes(root, nodeOpacities))
            {
                var node = nodeWorldTransformed.node;
                if (!nodeGeoms.ContainsKey(node))
                    continue;
                result.AddRange(nodeGeoms[node]);
            }

            return result;
        }

        private static void TessellateShape(Shape vectorShape, List<Geometry> geoms, TessellationOptions tessellationOptions)
        {
            if (vectorShape.fill != null)
            {
        ...
        public static Texture2D GenerateAtlasAndFillItsUVs(IEnumerable<Geometry> geoms, uint rasterSize)
        {
            ...
            var atlasTex = new Texture2D(atlasWidth, atlasHeight, TextureFormat.ARGB32, false, false);
