...
namespace UnityEngine.XR.ARFoundation
{
        ...
        ARPointCloud>, IRaycaster
    {
        ...
        {
            pointCloud.m_Points.Clear();
            pointCloud.m_Confidence.Clear();
            pointCloud.m_Ids.Clear();

            if (subsystem.SubsystemDescriptor.supportsFeaturePoints)
            {
                using (var nativePositions = subsystem.GetFeaturePointPositions(sessionRelativeData.trackableId, Allocator.Temp))
                {
                    foreach (var position in nativePositions)
                        pointCloud.m_Points.Add(position);
                }
            }

            if (subsystem.SubsystemDescriptor.supportsConfidence)
            {
                using (var nativeConfidence = subsystem.GetFeaturePointConfidence(sessionRelativeData.trackableId, Allocator.Temp))
                {
                    foreach (var confidenceValue in nativeConfidence)
                        pointCloud.m_Confidence.Add(confidenceValue);
                }
            }

            if (subsystem.SubsystemDescriptor.supportsUniqueIds)
            {
                using (var nativeIds = subsystem.GetFeaturePointIds(sessionRelativeData.trackableId, Allocator.Temp))
                {
                    foreach (var id in nativeIds)
                        pointCloud.m_Ids.Add(id);
                }
            }
        }
            ...
            Allocator allocator)
        {
            ...
            foreach (var pointCloud in trackableCollection)
            {
                ...
                var points = pointCloud.GetPoints(Allocator.TempJob);
