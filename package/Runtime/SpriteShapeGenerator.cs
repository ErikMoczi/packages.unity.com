...
namespace UnityEngine.U2D
{
    ...
    public struct SpriteShapeGenerator : IJob
    {
        ...
        [DeallocateOnJobCompletion]
        private NativeArray<int> m_SpriteIndices;
        ...
        unsafe void PrepareInput(SpriteShapeParameters shapeParams, int maxArrayCount, ShapeControlPointExperimental[] shapePoints, bool updateCollider, bool optimizeCollider, float colliderPivot, float colliderDetail)
        {
            ...
            m_SpriteIndices = new NativeArray<int>(kControlPointCount, Allocator.TempJob, NativeArrayOptions.UninitializedMemory);
        ...
        int GetSpriteIndex(int index, int previousIndex)
        {
            int next = (index + 1) % controlPointCount, spriteIndex = -1, firstSpriteIndex = -1;
            float order = 0;
            float angle = SlopeAngle(GetControlPoint(next).position, GetControlPoint(index).position);
            bool resolve = ResolveAngle(angle, GetControlPoint(index).cpData.x, ref order, ref spriteIndex, ref firstSpriteIndex);
        ...
        bool GenerateControlPoints()
        {
            ...
            for (int i = 0; i < controlPointCount; ++i)
            {
                int spriteIndex = GetSpriteIndex(i, activeIndex);
                m_SpriteIndices[i] = activeIndex = spriteIndex;
            }
        ...
        bool InsertCorner(int index, ref NativeArray<int> cpSpriteIndices, ref NativeArray<JobControlPoint> newPoints, ref int activePoint)
        {
            int i = (index == 0) ? (controlPointCount - 1) : (index - 1);
            int k = (index + 1) % controlPointCount;

            // Check if we have valid Sprites.
            if (cpSpriteIndices[i] >= spriteCount || cpSpriteIndices[index] >= spriteCount)
                return false;
            ...
            JobSpriteInfo psi = GetSpriteInfo(cpSpriteIndices[i]);
            JobSpriteInfo isi = GetSpriteInfo(cpSpriteIndices[index]);

            // Check if the Sprites Height and Pivot matches. Otherwise not allowed.
            if (psi.uvInfo.w != isi.uvInfo.w || psi.metaInfo.y != 0.5f || psi.metaInfo.y != isi.metaInfo.y)
                return false;
