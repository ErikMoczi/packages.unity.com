...
namespace Unity.Rendering
{
    ...
    public class MeshInstanceRendererSystem : ComponentSystem
	{
	    ...
	    ComponentGroup              m_CustomLocalToWorldInstanceRendererGroup;
	    ComponentGroup              m_LocalToWorldInstanceRendererGroup;
	    ...
	    public unsafe static void CopyMatrices(ComponentDataArray<CustomLocalToWorld> transforms, int beginIndex, int length, Matrix4x4[] outMatrices)
	    {
	        // @TODO: This is using unsafe code because the Unity DrawInstances API takes a Matrix4x4[] instead of NativeArray.
	        // We want to use the ComponentDataArray.CopyTo method
	        // because internally it uses memcpy to copy the data,
	        ...
	        {
	            Assert.AreEqual(sizeof(Matrix4x4), sizeof(LocalToWorld));
	            var matricesSlice = NativeSliceUnsafeUtility.ConvertExistingDataToNativeSlice<CustomLocalToWorld>(matricesPtr, sizeof(Matrix4x4), length);
#if ENABLE_UNITY_COLLECTIONS_CHECKS
	            NativeSliceUnsafeUtility.SetAtomicSafetyHandle(ref matricesSlice, AtomicSafetyHandle.GetTempUnsafePtrSliceHandle());
#endif
	            transforms.CopyTo(matricesSlice, beginIndex);
	    ...
	    protected override void OnCreateManager(int capacity)
	    {
	        ...
	        m_LocalToWorldInstanceRendererGroup = GetComponentGroup(typeof(MeshInstanceRenderer), typeof(LocalToWorld), ComponentType.Subtractive<MeshCulledComponent>(), ComponentType.Subtractive<MeshLODInactive>());
	        m_CustomLocalToWorldInstanceRendererGroup = GetComponentGroup(typeof(MeshInstanceRenderer), typeof(CustomLocalToWorld), ComponentType.Subtractive<MeshCulledComponent>(), ComponentType.Subtractive<MeshLODInactive>());
	    }

	    void UpdateLocalToWorldInstanceRenderer()
	    {
		    var forEachFilter = m_LocalToWorldInstanceRendererGroup.CreateForEachFilter(m_CacheduniqueRendererTypes);
	        var count = m_LocalToWorldInstanceRendererGroup.CalculateLength();
            for (int i = 0;i != m_CacheduniqueRendererTypes.Count;i++)
            {
                ...
                var transforms = m_LocalToWorldInstanceRendererGroup.GetComponentDataArray<LocalToWorld>(forEachFilter, i);
                ...
                // So we just have a for loop here, representing each Graphics.DrawMeshInstanced batch
                {
                    ...
                    while (beginIndex < transforms.Length)
                    {
                        ...
                        Graphics.DrawMeshInstanced(renderer.mesh, renderer.subMesh, renderer.material, m_MatricesArray,
                            length, null, renderer.castShadows, renderer.receiveShadows);
	    ...
	    void UpdateCustomLocalToWorldInstanceRenderer()
	    {
		    var forEachFilter = m_CustomLocalToWorldInstanceRendererGroup.CreateForEachFilter(m_CacheduniqueRendererTypes);
	        var count = m_CustomLocalToWorldInstanceRendererGroup.CalculateLength();
            for (int i = 0;i != m_CacheduniqueRendererTypes.Count;i++)
            {
                // For each unique MeshInstanceRenderer data, we want to get all entities with a CustomLocalToWorld
                // SharedComponentData gurantees that all those entities are packed togehter in a chunk with linear memory layout.
                // As a result the copy of the matrices out is internally done via memcpy.
                var renderer = m_CacheduniqueRendererTypes[i];
                var transforms = m_CustomLocalToWorldInstanceRendererGroup.GetComponentDataArray<CustomLocalToWorld>(forEachFilter, i);
	    ...
	    protected override void OnUpdate()
		{
		    ...
		    UpdateLocalToWorldInstanceRenderer();
		    UpdateCustomLocalToWorldInstanceRenderer();
